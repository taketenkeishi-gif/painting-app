...
# ── Configure + Build (inside vcvars64 if found) ─────────────────────────────
$configureCmd = "`"$CMakePath`" -S `"$repoRoot`" -B `"$repoRoot\$BuildDir`" -G Ninja `"-DCMAKE_PREFIX_PATH=$QtPrefix`" -DPAINT_BUILD_TESTS=OFF `"-DCMAKE_BUILD_TYPE=$Config`""
$buildCmd     = "`"$CMakePath`" --build `"$repoRoot\$BuildDir`" --config $Config"

if ($vcvarsPath) {
  $combined = "call `"$vcvarsPath`" && $configureCmd && $buildCmd"
  cmd /c $combined
