...
void LineTool::addVectorLine(Layer& layer, const Point& from, const Point& to, const Color& color, int size) const {
  VectorPath path;
  path.points.push_back(FPoint {static_cast<float>(from.x), static_cast<float>(from.y)});
  path.points.push_back(FPoint {static_cast<float>(to.x), static_cast<float>(to.y)});
  path.color = color;
